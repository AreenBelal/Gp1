...
import { getAllStudentNotifications } from "../../system-redux/users/students/allStudentNotificationsSlice";
...
const StudentNotificationsMenu = () => {
  const { studentData, studentLoading } = useSelector(
    (state) => state.singleStudent
  );
  const { allStudentNotificationsData, allStudentNotificationsLoading } =
    useSelector((state) => state.allStudentNotifications);
  console.log("test studentData:", studentData);
  ...
  useEffect(() => {
    if (studentData?._id) {
      dispatch(getAllStudentNotifications(studentData?._id));
    }
    console.log(allStudentNotificationsData);
  }, [studentData, updatePage]);
  ...
  const unread = async (notificationId) => {
    ...
    try {
      const option = {
        method: "PATCH",
        url: `${base_url}/students/notifications/${notificationId}`,
  ...
  const deleteAll = async () => {
    ...
    try {
      const option = {
        method: "GET",
        url: `${base_url}/students/notifications/delete-or-update/${studentData?._id}?deleteAll=all`,
        // headers: { Authorization: `Bearer ${userData?.token}` },
      };
  ...
  const readAll = async () => {
    ...
    try {
      const option = {
        method: "GET",
        url: `${base_url}/students/notifications/delete-or-update/${studentData?._id}?seen=all`,
        // headers: { Authorization: `Bearer ${userData?.token}` },
      };
  ...
  const counter = allStudentNotificationsData?.filter(
    ...
    <Menu>
      <MenuHandler>
        <IconButton variant="text">
          <Badge
            content={
              allStudentNotificationsLoading ? (
      ...
      <MenuList className="flex flex-col gap-2" dir="rtl">
        {allStudentNotificationsData.length > 0 && (
        ...
        {allStudentNotificationsLoading ? (
          ...
          allStudentNotificationsData?.map((notification, index) => (
            <MenuItem
              key={index}
...
export default StudentNotificationsMenu;
