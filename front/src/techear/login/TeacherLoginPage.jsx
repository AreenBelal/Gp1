...
const TeacherLoginPage = () => {
  ...
  const { cn, tc } = useSelector((state) => state.ma);
  ...
  const onLogin = async (ev) => {
    ...
    try {
      setLoading(true);
      const response = await axios.post(`${base_url}/teachers/login`, user, {
      ...
      toast.error(
        `${error?.response?.data?.message}` ||
          "فشل تسجيل الدخول الرجاء المحاولة مرة اخري"
      );
    } finally {
      setLoading(false);
    }
      ...
      >
        ...
        >
          ...
          >
            ...
            <span className="text-mainColor500 font-bold">كمعلم</span>
          </Typography>
          <Typography
            color="gray"
            className={`mt-1 font-normal ${typographyStyle}`}
          >
            سعيد بلقائك! هل تريد تسجيل الدخول كمعلم؟ املء هذه البيانات
            التالية...
          </Typography>
          <form
            onSubmit={onLogin}
          ...
          >
            ...
            <Typography
              color="gray"
              className={`${typographyStyle} mt-4 text-center font-normal`}
            >
              <span>ليس لديك إيميل؟ </span>
              <Link
                to={"/#contact"}
                className={`font-medium text-gray-900 ${typographyStyle} dark:hover:text-darkMode-dark400`}
              >
                سجل الآن كمعلم علي منصة تفوَّق
              </Link>
            </Typography>
          </form>
        </Card>
