...
import { getAllTeacherNotifications } from "../../system-redux/users/teachers/allTeacherNotificationsSlice";
...
const TeacherNotificationsMenu = () => {
  const { teacherData, teacherLoading } = useSelector(
    (state) => state.singleTeacher
  );
  const { allTeacherNotificationsData, allTeacherNotificationsLoading } =
    useSelector((state) => state.allTeacherNotifications);
  console.log("test teacherData:", teacherData);
  ...
  useEffect(() => {
    if (teacherData?._id) {
      dispatch(getAllTeacherNotifications(teacherData?._id));
    }
    console.log(allTeacherNotificationsData);
  }, [teacherData, updatePage]);
  ...
  const unread = async (notificationId) => {
    ...
    try {
      const option = {
        method: "PATCH",
        url: `${base_url}/teachers/notifications/${notificationId}`,
  ...
  const deleteAll = async () => {
    ...
    try {
      const option = {
        method: "GET",
        url: `${base_url}/teachers/notifications/delete-or-update/${teacherData?._id}?deleteAll=all`,
        // headers: { Authorization: `Bearer ${userData?.token}` },
      };
  ...
  const readAll = async () => {
    ...
    try {
      const option = {
        method: "GET",
        url: `${base_url}/teachers/notifications/delete-or-update/${teacherData?._id}?seen=all`,
        // headers: { Authorization: `Bearer ${userData?.token}` },
      };
  ...
  const counter = allTeacherNotificationsData?.filter(
    ...
    <Menu>
      <MenuHandler>
        <IconButton variant="text">
          <Badge
            content={
              allTeacherNotificationsLoading ? (
      ...
      <MenuList className="flex flex-col gap-2" dir="rtl">
        {allTeacherNotificationsData.length > 0 && (
        ...
        {allTeacherNotificationsLoading ? (
          ...
          allTeacherNotificationsData?.map((notification, index) => (
            <MenuItem
              key={index}
...
export default TeacherNotificationsMenu;
